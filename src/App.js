...
function App() {
  ...
  useEffect(() => {
    async function fetchPokemonNames() {
      ...
      result.pokemon_entries.forEach((pokemon) =>
    ...
    Promise.all([fetchPokemonNames(), fetchMoveNames(), fetchTypeData()]).then(
      () => {
        setIsLoaded(true);
      },
      (error) => {
    ...
    teamValue.forEach((member, index) => {
      if (member.data && !member.checked) {
        ...
        member.data.types.forEach((type) => {
          const found = typeData.find((value) => value.name === type);
          if (found) {
            // Double damage
            found.damage_relations.double_damage_from.forEach(
              (damagingType) => {
                ...
                if (!typeTracker[damagingType.name].decremented) {
                  ...
                  tempDamage[damagingType.name]
                    ? tempDamage[damagingType.name]--
                    : (tempDamage[damagingType.name] = -1);
                ...
                tempDamageValues.defensive[damagingType.name].popover[
                  index
                ] *= 2;
              }
            );
        ...
        member.moves.forEach((move) => {
          if (move.data && !move.checked) {
            ...
            if (
              ...
              const found = typeData.find(
                (value) => value.name === move.data.type.name
              );
              if (found) {
                found.damage_relations.double_damage_to.forEach(
                  (damagedType) => {
                    // Sets damage value in type chart
                    tempDamageValues.offensive[damagedType.name].type_chart++;
                    // Sets damage value of damaged type for each move
                    tempDamage[damagedType.name]
                      ? tempDamage[damagedType.name]++
                      : (tempDamage[damagedType.name] = 1);
                  }
                );
              }
            }
